...
The deep-link helper's 900ms compatibility fallback now records the thread generation at click time and invalidates only if no authoritative post-send synchronization occurred. This removes the redundant second thread refetch after a successful synchronized send while preserving fallback behavior for the legacy base handler. Focused V1.4L regression is 16/16 green; the context browser contract is 8/8 green.
## Round 5 — Manager / Platform / Ads read-only workspace sweep

- Dhanbad Local Manager 02 was exercised read-only across Overview, People, Learning, Reports, Ads Review, Location Settings, Raahi Desk and Founding Supply. Operational scope remained fixed to Dhanbad; the People page correctly refused to become a learner directory; Founding Supply showed no manufactured requests; Raahi Desk clearly identifies Platform-authored content and was not used to seed synthetic activity.
- Platform Admin 08 switched into Platform operations and opened Overview, Safety, Ads Ops and Audit. Platform authority was server-projected from `platform_admin`; no privileged state was mutated during the sweep.
- The Platform Audit route was functional but exposed each row as raw JSON. Repair candidate now renders human action/target/time summaries first while retaining IDs and metadata under expandable Technical details.
- Platform Admin 08 also exercised Raahi Ads read-only: Campaigns, eligibility, campaign-create form, inventory and aggregate analytics. No fake campaign or inventory reservation was created.
- Raahi Ads exposed an Organization navigation item even when this Account had no Organization, leading to an empty Institute page. Repair candidate now shows the Organization link only when an Organization context exists; account-owned advertiser flows remain available.
- Learning Opportunities remains empty in Dhanbad. No synthetic Learning Request was created merely to exercise the workflow.
- Focused V1.4L regression is now 18/18 green; sealed browser contract chain exits successfully. No repair adds browser table DML or new authority.

Next gate: commit/qualify/deploy this presentation-only admin batch, verify Platform Audit and Ads navigation live, then continue remaining non-synthetic actor/workspace checks.