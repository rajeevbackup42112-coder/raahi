...
- GitHub Browser Contract #21 exposed a genuine fast-load race: the async base bootstrap could paint and submit Institute setup before `live-product-fix-v13.js` had registered its early-submit guard. The build now injects the product-fix script immediately after the app root, before the async Supabase/live bootstrap begins. The context browser contract then passed five consecutive local runs; this ordering is also regression-tested.
