(() => {
  ...
  const wait = setInterval(() => {
    ...
    function pagePhoneCheck() {
      ...
      if (!trust) return `<div class="auth-shell"><div class="auth-card"><div class="eyebrow">Security check</div><h1>Checking your phone…</h1><p class="muted">Raahi is checking whether your recent phone confirmation is still valid.</p><button class="ghost-btn wide" data-live-fix-logout>Log out</button></div></div>`;
      if (trust.state === 'fresh') return `<div class="auth-shell"><div class="auth-card"><div class="eyebrow">Security check</div><h1>Phone confirmed</h1><p class="muted">You’re ready to continue.</p><button class="primary-btn wide" data-live-fix-resume-action>Continue</button><button class="ghost-btn wide" data-live-fix-logout>Log out</button></div></div>`;
